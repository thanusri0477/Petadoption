...
otpSchema.statics.genOtp = async function (name, email, password) {
  ...
  if (existingOtp && existingOtp.expiresAt > Date.now()) {
    ...
    throw new Error(`An OTP has already been sent. Please wait ${minutesRemaining} minute(s) before requesting again.`);
  }
  ...
  await this.deleteOne({ email }); // Remove existing OTP before creating a new one
  ...
  const mailOptions = {
    ...
    to: email,
    subject: 'Home4Paws - Your OTP Code',
...
otpSchema.statics.forgotOtp = async function (email) {
  ...
  const mailOptions = {
    from: process.env.EMAIL_USER,
    to: email,
    subject: 'Home4Paws - Reset Your Password',
